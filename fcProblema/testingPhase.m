function [TESTPHASE,RESULTS] = testingPhase(d,kernel,RESULTS,TESTPHASE,PARAMETERS,DATASET,STR)
...
try
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(1,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(1,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(1,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(1,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(1,6);
    end
    ...
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(1,2);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(2,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(2,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(2,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(2,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(2,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(2,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(3,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(3,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(3,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(3,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(3,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(3,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(4,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(4,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(4,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(4,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(4,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(4,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(5,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(5,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(5,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(5,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(5,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(5,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(6,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(6,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(6,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(6,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(6,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(6,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(7,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(7,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(7,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(7,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(7,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(7,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(8,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(8,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(8,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(8,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(8,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(8,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(9,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(9,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(9,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(9,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(9,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(9,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(11,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(11,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(11,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(11,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(11,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(11,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'rbf')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(12,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'linear')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(12,1);% *** calcula a matriz kernel da fase de treino
    elseif strcmp(PARAMETERS.chosenKernel,'poly')
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(12,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS.d = RESULTS.bestParameters{d,kernel}(12,5);
        PARAMETERS.r =RESULTS.bestParameters{d,kernel}(12,6);
    end
    PARAMETERS = makingStructs(PARAMETERS,DATASET);
    ...
    % *** cria o limite superior da caixa e configura as opcoes do libsvm
    PARAMETERS.C = RESULTS.bestParameters{d,kernel}(12,2);
    PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
...
try
    ...
    if strcmp(PARAMETERS.chosenKernel,'linear') 
        PARAMETERS.gammaKernel = RESULTS.bestParameters{d,kernel}(13,1);% *** calcula a matriz kernel da fase de treino
        PARAMETERS = makingStructs(PARAMETERS,DATASET);
        ...
        % *** cria o limite superior da caixa e configura as opcoes do libsvm
        PARAMETERS.C = RESULTS.bestParameters{d,kernel}(13,2);
        PARAMETERS.ub = PARAMETERS.C*ones(DATASET.n_train,1);
